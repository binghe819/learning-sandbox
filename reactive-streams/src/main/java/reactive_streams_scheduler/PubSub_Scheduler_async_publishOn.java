 ...
 *
 * TO-BE:  Scheduler를 이용해서 각 아이템을 비동기적으로 실행한다. -> publishOn 방식
 *
 * Subscriber가 onNext()를 받아서 처리할 때, 매 onNext()마다 새로운 스레드에서 실행된다.
 *
 */
public class PubSub_Scheduler_async_publishOn {

    private static Logger log = LoggerFactory.getLogger(PubSub_Scheduler_async_publishOn.class);
