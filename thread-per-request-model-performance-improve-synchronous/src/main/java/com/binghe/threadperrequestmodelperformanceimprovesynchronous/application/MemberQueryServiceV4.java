 ...
 * - 비동기 Task의 예외 처리
 *   - 비동기 Task에서 예외 발생시 HTTP 요청 자체가 예외를 던져야할 경우, 비동기 Task에 대한 예외 처리를 하지 않으면, get(), join()호출시 예외가 호출자 스레드까지 전파된다. (get 메서드 명세 참고)
 *   - 비동기 Task에서 예외 발생시 특정 값으로 복구하고싶다면 CompletableFuture에서 제공하는 예외 처리 로직을 활용하면 된다. (이 경우 호출자 스레드까지 예외가 전파되지 않는다.)
 */
@RequiredArgsConstructor
@Service
public class MemberQueryServiceV3 {

    private final ExecutorService asyncExecutorService;

    private final MemberRepository memberRepository;
    private final CheckBalance dossBankCheckBalanceV3;
    private final CheckBalance kakaokBankCheckBalanceV3;
    ...
    public CheckMemberBalanceResponse checkMemberBalances(Long memberId) {
        ...
        CompletableFuture<CheckBalanceResult> dossBalanceCompletableFuture = CompletableFuture
                .supplyAsync(() -> dossBankCheckBalanceV3.checkBalance(memberId), asyncExecutorService)
        ...
        CompletableFuture<CheckBalanceResult> kakaokBalanceCompletableFuture = CompletableFuture
                .supplyAsync(() -> kakaokBankCheckBalanceV3.checkBalance(memberId), asyncExecutorService)
                .exceptionally(ex -> CheckBalanceResult.EMPTY);

        CompletableFuture<CheckBalanceResult> shinhenBalanceCompletableFuture = CompletableFuture
                .supplyAsync(() -> shinhenBankCheckBalanceV3.checkBalance(memberId), asyncExecutorService)
                .exceptionally(ex -> CheckBalanceResult.EMPTY);
